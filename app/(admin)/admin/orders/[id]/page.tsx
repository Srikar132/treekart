  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex flex-col gap-4">
        <Link
          href="/admin/orders"
          className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-primary transition-colors group w-fit"
        >
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <h1 className="text-2xl font-black text-slate-900 uppercase tracking-tight">Order Manifest</h1>
        ...
        <div className="lg:col-span-8 space-y-8">
          ...
          <div className="data-card">
            <div className="flex items-center gap-3 mb-8">
              ...
              <h3 className="text-sm font-black text-slate-900 uppercase">Fulfillment Pipeline</h3>
            </div>
            ...
            <div className="divide-y divide-slate-50">
              {items.map((item, idx) => (
                <div key={idx} className="p-6 flex items-center gap-6">
                  <div className="h-16 w-16 bg-slate-100 rounded-xl overflow-hidden border border-slate-200 shrink-0">
                    {item.imageUrl ? (
                  ...
                  <div className="text-right">
                    <p className="text-sm font-black text-slate-900">₹{item.lineTotal.toLocaleString()}</p>
                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">₹{item.pricePerKg}/kg</p>
                  </div>
                </div>
              ))}
            </div>
            <div className="p-8 bg-slate-50/50 border-t border-slate-100">
              <div className="max-w-[240px] ml-auto space-y-3">
                <div className="flex justify-between text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                  <span>Subtotal</span>
                  <span className="text-slate-900 font-black">₹{order.total_amount.toLocaleString()}</span>
                </div>
                <div className="flex justify-between text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                  <span>Logistics</span>
                ...
                <div className="flex justify-between items-baseline pt-2">
                  ...
                  <span className="text-2xl font-black text-slate-900 tracking-tighter">₹{order.total_amount.toLocaleString()}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <aside className="lg:col-span-4 space-y-8">
          ...
          <div className="data-card">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 className="text-sm font-black text-slate-900 uppercase">Customer Profile</h3>
            </div>
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-xs font-bold text-slate-900 uppercase">{order.profiles?.phone || 'No Phone'}</p>
              </div>
            </div>
          </div>
          ...
          <div className="data-card">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 className="text-sm font-black text-slate-900 uppercase">Delivery Logistics</h3>
            </div>
            <div className="space-y-4">
              <div>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Recipient Name</p>
                <p className="text-xs font-bold text-slate-900 uppercase">{delivery?.name}</p>
              </div>
              <div>
          ...
          <div className="data-card">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 className="text-sm font-black text-slate-900 uppercase">Transaction Data</h3>
            </div>
            <div className="space-y-6">
              <div>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Payment ID</p>
                <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 font-mono text-[10px] text-slate-500 break-all">
                  {order.payment_id}
                </div>
              </div>
              {order.tracking_id && (
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Tracking Index</p>
                  <div className="flex items-center justify-between bg-white p-3 rounded-xl border border-primary/20 shadow-sm shadow-primary/5">
